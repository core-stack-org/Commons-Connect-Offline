    ...
    const fetchResourcesLayers = async() =>{
        ...
        if(submissions !== null && submissions["waterstructure"] !== undefined){
            submissions["waterstructure"].map(async(formData) =>{
                ...
                await assetsLayerRefs[2].current.getSource().addFeature(tempFeature)
            })
        }

        if(submissions !== null && submissions["cropping"] !== undefined){
            submissions["cropping"].map(async(formData) =>{
                const tempFeature = new Feature({
                    geometry : new Point([formData.GPS_point["longitude"], formData.GPS_point["latitude"]]),
                    ...formData
                });
                ...
                tempFeature.setStyle(
                    new Style({
                      image:
                        new Icon({ src: CroppingIcon})
                    })
                )
                await assetsLayerRefs[3].current.getSource().addFeature(tempFeature)
            })
        }
    ...
    const refreshResourceLayers = async() => {
        ...
        if(currentScreen === "Resource_mapping"){
            ...
            if(currentStep === 0){
                ...
                const submissions = await JSON.parse(localStorage.getItem(MainStore.currentPlan.id));
                let formData = submissions["cropping"][submissions["cropping"].length - 1]
